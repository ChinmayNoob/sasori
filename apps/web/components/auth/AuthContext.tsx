...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const clearAuth = () => {
    localStorage.removeItem("sasori_auth_token");
    setToken(null);
    setUser(null);
  };
  ...
  useEffect(() => {
    ...
    const tokenToValidate = urlToken ?? localStorage.getItem("sasori_auth_token");
    ...
    fetch(`${API_URL}/auth/me`, {
      ...
      .then(async res => {
        ...
        if (urlToken) {
          localStorage.setItem("sasori_auth_token", urlToken);
          router.replace("/chat");
        }
      ...
      .catch(() => {
        ...
        const storedToken = localStorage.getItem("sasori_auth_token");
        if (storedToken) {
